...
- `/new` = not-yet-graduated tokens sorted by **closest to graduate** (progress %), with progress bar from the API
...
| New (bonding) tokens | `GET /tokens?status=new&sort=volume|newest` (board merges scans + client-sorts by `graduationProgress` desc; API has no progress sort) |
